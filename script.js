window.onload = function() {
    const btnClicked = document.getElementById("buttonClicked");
    ...
    const errorState = (numberOfError, typeOfData, typeOfError, color)=>{
        error[numberOfError].innerHTML = typeOfError;
        labels[numberOfError].style.color = color;
        typeOfData.style.borderColor = color;
    }
...
const isYearCorrect = () => {
    if(year.value == ""){
        ...
        errorState(0, month , typeOfError[2], "#ff5757");
        return false;
    }
    else{
        errorState(2, month , typeOfError[0], "");
        return true;
    }
}
